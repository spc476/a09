...
static bool fbasic_cmdline(struct format *fmt,struct a09 *a09,struct arg *arg,char c)
{
  ...
  switch(c)
  {
    case 'C':
         ...
         {
           fprintf(stderr,OPT "C: line number must be between 1 and 63999\n");
           return false;
         }
         break;
    ...
    case 'N':
         ...
         {
           fprintf(stderr,OPT "N: line increment must be between 1 and 65535\n");
           return false;
         }
         break;
...
static bool fbasic_end(struct format *fmt,struct opcdata *opd,struct symbol const *sym)
{
  ...
  if (opd->pass == 2)
  {
    ...
    if (defusr)
    {
      ...
      fwrite(basic->buffer,1,basic->idx,opd->a09->out);
    }
