...
static int usage(char const *prog)
{
  fprintf(
           ...
           OC "M\t\tgenerate Makefile dependencies to stdout\n"
           OC "T\t\trun tests with TAP output to stdout\n"
           ...
           "\n"
           "\tIf no input file given, code read from stdin\n"
           "\tIf no output file given, code written to stdout\n"
           "\n"
           "\tformat: bin rsdos srec basic dragon\n"
           "\tnumlist: N1,N2,N3-N4\n"
           "\tdefine: name[=number]\n"
           "%s"
           "%s"
